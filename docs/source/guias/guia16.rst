...
.. note::
    ...
    Elabora una estrategia para utilizar el `localstorage` como almacenamiento temporal de la respuesta a un requerimiento asincrónico. Considere que la estrategia debe combinar la eficiencia (menos llamadas a la API), resiliencia (uso de datos en caso de falla) y vigencia de la información (control temporal de *x* minutos).
...
3. Sin utilizar IAG, diseñe e implemente la estrategia de persistencia de datos en su aplicación React, asegurando que los datos se almacenen en `localStorage` y se sincronicen con el estado de la aplicación.
